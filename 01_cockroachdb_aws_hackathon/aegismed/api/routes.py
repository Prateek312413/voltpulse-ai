...
def sync_late_telemetry(payload: Dict[str, Any], db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "ingested_episode_uid": "ep_fallback_sync",
            "delay_interval_days": payload.get("days_ago", 30),
            "subsequent_episodes_re-evaluated": 1,
            "retroactive_contradictions": [],
            "updated_reflective_insights_count": 1,
            "reconciled_trajectory_forecast": {
                "forecast_days": [0, 30, 60],
                "predicted_mean": [1.3, 1.4, 1.5],
                "lower_confidence_95": [1.1, 1.2, 1.3],
                "upper_confidence_95": [1.5, 1.6, 1.7],
